...
export default function NavigationComponent({ site = false }: NavbarProps) {
  ...
  const { previousUrls, pushUrl, popUrl, user } = useMainStore();
    ...
    >
      <Dock className="pointer-events-auto relative z-50 mx-auto flex h-full items-center bg-background px-1 shadow-md dark:border dark:shadow-xl">
        {previousUrls.length > 0 && (
          <motion.div variants={buttonVariants} initial="hidden" animate="visible" transition={{ duration: 0.3, delay: 0.1 }}>
            <Button className="mr-2 rounded-full p-4" onClick={handleGoBack}>
              Go Back
            </Button>
          </motion.div>
        )}
        ...
        {site ? (
          <div className="flex flex-row">
            <motion.div variants={buttonVariants} initial="hidden" animate="visible" transition={{ duration: 0.3, delay: 0.2 }}>
              <Button className="rounded-xl bg-[#F9F9F9] p-2 text-black hover:text-white" onClick={() => handleAddSection("textbox")}>
                Add Text
              </Button>
            </motion.div>
            <motion.div variants={buttonVariants} initial="hidden" animate="visible" transition={{ duration: 0.3, delay: 0.3 }}>
              <Button className="rounded-xl bg-[#F9F9F9] p-2 text-black hover:text-white" onClick={() => handleAddSection("media")}>
                Add Images
              </Button>
            </motion.div>
            <motion.div variants={buttonVariants} initial="hidden" animate="visible" transition={{ duration: 0.3, delay: 0.4 }}>
              <Button className="rounded-xl bg-[#F9F9F9] p-2 text-black hover:text-white" onClick={() => handleAddSection("media")}>
                Add Article
              </Button>
            </motion.div>
            <motion.div variants={buttonVariants} initial="hidden" animate="visible" transition={{ duration: 0.3, delay: 0.5 }}>
              <Button className="flex items-center rounded-full bg-[#F9F9F9] p-2 text-black" onClick={() => handleNavigation(`${basePath}/settings`)}>
                <SettingsIcon />
              </Button>
            </motion.div>
            <motion.div variants={buttonVariants} initial="hidden" animate="visible" transition={{ duration: 0.3, delay: 0.6 }}>
              <Button className="flex items-center rounded-full bg-[#F9F9F9] p-2 text-black" onClick={() => handleNavigation(`${basePath}/analytics`)}>
                <AnalyticsIcon />
              </Button>
            </motion.div>
          </div>
        ) : settings ? (
          <motion.div className="flex flex-row gap-2" variants={buttonVariants} initial="hidden" animate="visible" transition={{ duration: 0.3, delay: 0.2 }}>
            <Button className="flex items-center rounded-full bg-[#F9F9F9] p-2 text-black outline-1 outline-black hover:bg-white" onClick={() => handleNavigation(`/site/${user.siteId}`)}>
          ...
          <div className="flex flex-row gap-2 px-1">
            ...
            <motion.div variants={buttonVariants} initial="hidden" animate="visible" transition={{ duration: 0.3, delay: 0.4 }}>
              <Button className="flex items-center rounded-full bg-[#F9F9F9] p-2 text-black outline-1 outline-black hover:bg-white" onClick={() => handleNavigation(`/settings`)}>
                <SettingsIcon />
              </Button>
            </motion.div>
          </div>
