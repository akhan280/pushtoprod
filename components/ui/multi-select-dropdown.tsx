...
//   /**
//    * The currently selected values to be displayed in the multi-select component.
...
>(
  (
    ...
    const toggleAll = () => {
      if (selectedSections.length === options.length) {
        handleClear();
      } else {
      ...
      >
        <PopoverTrigger asChild>
          <Button
            ...
            onClick={handleTogglePopover}
            className={cn(
              "flex p-1 rounded-md border bg-background hover:bg-inherit",
              className
            )}
          >
              ...
              <CommandGroup>
                ...
                >
                  <div
                    className={cn(
                      "mr-2 flex h-4 w-4 items-center justify-center rounded-sm border border-primary",
                      selectedSections.length === options.length
                        ? "bg-primary text-primary-foreground"
                        : "opacity-50 [&_svg]:invisible"
                    )}
                  >
                ...
                {options.map((option) => {
                  const isSelected = selectedSections.includes(option.value);
                  return (
                    <CommandItem
                      key={option.value}
                      onSelect={() => toggleOption(option.value)}
